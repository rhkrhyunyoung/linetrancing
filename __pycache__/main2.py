...
def main():
    ...
    if not cap.isOpened():
        print(f"🚨 에러: {camera_index}번 웹캠을 열 수 없습니다! 인덱스를 확인하세요.")
        return
    ...
    try:
        while rclpy.ok():
            ...
            if smoothed_left is not None and smoothed_right is not None:
                ...
                target_linear_x = 0.4
                target_angular_z = float(-error * 0.003)
                ...
                final_angular_z = target_angular_z + slip_correction
                # -------------------------------
                
                twist.linear.x = float(target_linear_x)
                twist.angular.z = float(final_angular_z)
                
                cmd_pub.publish(twist)

                # 화면에 정보 표시 (IMU 값도 추가)
                cv2.putText(display_bev, f"Error: {error:.1f} px", (10, 30), 1, 1.2, (0, 255, 0), 2)
                cv2.putText(display_bev, f"Cmd Linear X: {target_linear_x:.2f} m/s", (10, 60), 1, 1.2, (255, 255, 255), 2)
                cv2.putText(display_bev, f"Cmd Angular Z: {final_angular_z:.2f} rad/s", (10, 90), 1, 1.2, (255, 255, 0), 2)
                cv2.putText(display_bev, f"Real IMU Z: {current_yaw_rate:.2f} rad/s", (10, 120), 1, 1.2, (0, 255, 255), 2)
